    ...
    const connectRealtimeNotifications = async () => {
      ...
      try {
        ...
        socket.on('connect', () => {
          ...
          console.debug('[client][admin] socket connected', { socketId: socket.id });
          setNotifLoading(false);
          setNotifError('');
        });
        ...
        socket.on('notifications:data', (payload) => {
          if (!active) return;
          // eslint-disable-next-line no-console
          console.debug('[client][admin] notifications:data', { count: Array.isArray(payload) ? payload.length : 0 });
        ...
        socket.on('reports:data', (payload) => {
          if (!active) return;
          // eslint-disable-next-line no-console
          console.debug('[client][admin] reports:data', { count: Array.isArray(payload) ? payload.length : 0 });
        ...
        socket.on('connect_error', (err) => {
          if (!active) return;
          // eslint-disable-next-line no-console
          console.warn('[client][admin] socket connect_error', err && err.message ? err.message : err);
          setNotifLoading(false);
          setNotifError('Realtime notifications unavailable.');
        });
        ...
        socket.on('disconnect', (reason) => {
          if (!active) return;
          // eslint-disable-next-line no-console
          console.info('[client][admin] socket disconnected', { reason });
        });
      } catch {
        if (!active) return;
        setNotifLoading(false);
